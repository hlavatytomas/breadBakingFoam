...
defineTypeNameAndDebug(unsBreadBakingSolid, 0);
addToRunTimeSelectionTable
(
    solidModel, unsBreadBakingSolid, dictionary
);
...
scalar unsBreadBakingSolid::residual(const volVectorField& D) const
{
    return
        gMax
        (
...
unsBreadBakingSolid::unsBreadBakingSolid
(
    Time& runTime,
    const word& region
)
...
bool unsBreadBakingSolid::evolve()
{
    ...
    do
    {
        ...
        volScalarField deltaP = pG - dimensionedScalar("p", dimPressure, 1e5);
...
tmp<vectorField> unsBreadBakingSolid::tractionBoundarySnGrad
(
